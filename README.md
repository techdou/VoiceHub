...
- 按键映射：RC003 实有 7 键（触摸板导航、音量、Home、菜单）绑定快捷键、媒体控制、音量、打开应用、删除整行等动作，支持单击、双击和长按。
- 按键映射改动即时保存生效，无需手动点保存；内置"删除整行"（Home → Shift+End → Backspace）等编辑动作，音量键同样支持双击 / 长按槽（如"音量减单击退格、双击删整行"）。
- 使用统计：按键次数、语音会话数、累计时长、最长单次时长，支持今日、本周、近 7 天和全部范围。
- 连接与音频自检、虚拟声卡安装引导、托盘常驻和可选开机自启。
- 语音触发（录音键模式）：录音键可选"按住说话"（蓝牙音频直传，默认）或"免提"（按一下开始、再按结束，经电脑麦克风收音）；改动随设备连接页保存即时生效。
- F5 拦截：遥控器语音键在系统层是 F5，声枢在遥控器在线期间拦截全部 F5（含键盘 F5），防止语音键漏出刷新网页丢失输入框内容；可在连接页关闭（关闭后仅靠录音会话窗口兜底），刷新网页用 Ctrl+R 不受影响。
...
| 遥控器 | 小米蓝牙遥控器 2 Pro（RC003，按键链路按真机实测校准）；其他型号未验证 |
...
5. 在设备连接页点击“测试音频链路”，确认端点与识别链路工作正常。
...
- 遥控器固件限制单次语音流约 60 秒（实测固件忽略协议级续租命令）。开启"长录音（实验性）"后，超过 60 秒会自动断开并重续蓝牙音频，识别连续不断；若重续失败则在该处正常收尾，松开再按即可继续。
